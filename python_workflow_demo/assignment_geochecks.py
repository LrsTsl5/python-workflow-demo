...
def value_validation(df: pd.DataFrame):
    print("Validating values in DataFrame")

    # Remove rows with invalid values in any column
    df = df[df["waardes"] != "STRING_INSTEAD_OF_NUMBER"]

    # Define thresholds and invalid values
    large_number_threshold = 1000  # Example threshold for large numbers

    # Remove rows with large numbers in the 'waardes' column
    df = df[df["waardes"] < large_number_threshold]
